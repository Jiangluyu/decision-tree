...
def calc_info_gain_for_series(dataset, i, base_ent):
    best_info_gain = 0.0
    best_split_point = -1
    ...
    for split_point in split_point_list:
        ...
        new_ent = len(elt_dataset) / len(sorted_feature_list) * calc_shannon_ent(elt_dataset) \
                  + len(gt_dataset) / len(sorted_feature_list) * calc_shannon_ent(gt_dataset)
...
def calc_info_gain(dataset, feature_list, i, base_ent):
...
def choose_best_feature_to_split(dataset):
    ...
    for i in range(num_features):
        ...
        if isinstance(feature_list[0], str):
            info_gain = calc_info_gain(dataset, feature_list, i, base_ent)
        else:
            info_gain, new_split_point = calc_info_gain_for_series(dataset, i, base_ent)
